...
Этот Docker-образ предназначен только для CPU-запуска FoldX mutation variants. Он не содержит OpenFold GPU worker, CUDA, веса OpenFold, Triton cache или polling-контракт глобального сервера.
...
```powershell
docker run --rm `
  -v D:\foldx\foldx.exe:/foldx/foldx:ro `
...
This Docker image is only for CPU FoldX mutation variants. It does not include the OpenFold GPU worker, CUDA, OpenFold weights, Triton cache, or the global server polling contract.
...
- A FoldX executable on the host.
