...
def test_resolve_query_payload_rejects_missing_query_id():
    try:
        run_single_protein_comparison.resolve_query_payload(
            {"queries": {"mini_query": {"chains": []}}},
            "missing_query",
        )
...
def test_resolve_query_payload_rejects_empty_queries():
    try:
        run_single_protein_comparison.resolve_query_payload({"queries": {}}, None)
    except ValueError as exc:
        assert "must contain at least one query" in str(exc)
    else:
        raise AssertionError("Expected ValueError for empty queries")
