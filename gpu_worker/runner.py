...
def _sequence_context(molecule: dict[str, Any], mutation: PointMutation) -> str:
    sequence_id = molecule.get("sequence_id")
    if sequence_id is None:
        return f"chain {mutation.chain_id}"
    return f"chain {mutation.chain_id}, sequence_id {sequence_id}"


def _validate_mutation_position(
    sequence: str,
    mutation: PointMutation,
    molecule: dict[str, Any],
) -> None:
    if mutation.position_1based < 1 or mutation.position_1based > len(sequence):
        raise ValueError(
            f"Invalid mutation position {mutation.position_1based} for sequence length {len(sequence)} "
            f"({_sequence_context(molecule, mutation)})"
        )
...
def apply_point_mutation_to_molecules(
    ...
    for molecule in work:
        ...
        _validate_mutation_position(sequence, mutation, molecule)
