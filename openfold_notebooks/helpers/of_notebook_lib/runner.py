...
def run_prediction(
    ...
    *,
    ...
    try:
        ...
        if monitor is not None:
            monitor.record_stage(
                "query_written",
                f"queries={len(payload.get('queries', {}))}",
            )
        ...
        run_params = {
            ...
            "inference_ckpt_path": (
                str(inference_ckpt_path) if inference_ckpt_path else None
            ),
    ...
    return RunResult(
        ...
        return_code=return_code,
        resource_csv_path=(
            monitoring_artifacts.resource_csv_path if monitoring_artifacts else None
        ),
        stage_marks_path=(
            monitoring_artifacts.stage_marks_path if monitoring_artifacts else None
        ),
        monitor_plot_path=(
            monitoring_artifacts.monitor_plot_path if monitoring_artifacts else None
        ),
    )
