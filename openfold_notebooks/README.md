...
- `OpenFold3_FoldX_DDG_Panel.ipynb`: run a pure FoldX WT-plus-19-mutants panel over one mutable chain of a two-protein complex from `PDB ID`, with resume over cached case outputs, global FoldX ranking, and inline WT/FoldX structure comparison.
...
- the FoldX panel notebook writes `summary.json`, `rows.csv`, `ranking.csv`, and one case folder per mutation with cached local FoldX outputs
