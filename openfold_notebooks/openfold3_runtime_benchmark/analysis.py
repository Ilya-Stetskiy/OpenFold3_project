...
def event_rows_to_dataframe(rows: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    if "event" in frame.columns:
        frame["event"] = frame["event"].astype(str).str.lower()
...
def _stage_duration_seconds(events_df: pd.DataFrame, stage: str) -> float | None:
    ...
    stage_df = events_df[events_df["stage"].astype(str) == stage].copy()
    if stage_df.empty:
        return None

    if "event" in stage_df.columns:
        stage_df["event"] = stage_df["event"].astype(str).str.lower()

    if "duration_seconds" in stage_df.columns:
        durations = pd.to_numeric(
            stage_df.loc[stage_df["event"] == "end", "duration_seconds"],
            errors="coerce",
        ).dropna()
    ...
    starts = pd.to_numeric(
        stage_df.loc[stage_df["event"] == "start", "relative_seconds"],
        errors="coerce",
    ).dropna().tolist()
    ends = pd.to_numeric(
        stage_df.loc[stage_df["event"] == "end", "relative_seconds"],
        errors="coerce",
    ).dropna().tolist()
    if not starts or not ends:
        return None

    return float(max(0.0, max(ends) - min(starts)))
