...
def test_run_runtime_benchmark_accepts_string_mode_and_writes_partial_outputs(
    tmp_path: Path,
    monkeypatch,
) -> None:
    success_composition = extract_entry_composition(FIXTURE_ROOT / "2crb.cif")
    preview_df = compositions_to_dataframe([success_composition])

    monkeypatch.setattr(
        "openfold3_runtime_benchmark.orchestration.collect_entry_compositions",
        lambda *args, **kwargs: [success_composition],
    )
    monkeypatch.setattr(
        "openfold3_runtime_benchmark.orchestration.compositions_to_dataframe",
        lambda *args, **kwargs: preview_df.copy(),
    ...
    result = run_runtime_benchmark(
        ...
        modes="cold",
        output_root=tmp_path / "runtime_runs",
        pipeline_trace_pdb_ids=None,
    )

    assert result.case_results_csv_path.exists()
    assert (result.run_root / "case_results.partial.csv").exists()
    assert len(result.case_results_df) == 1
    row = result.case_results_df.iloc[0]
    assert row["run_mode"] == "cold"
    assert row["status"] == "failed"
    assert "synthetic orchestration failure" in row["failure_reason"]
    assert row["log_path"].endswith("run_error.txt")
