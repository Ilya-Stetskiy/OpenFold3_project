...
def test_runtime_config_defaults_follow_current_project_dir(monkeypatch, tmp_path: Path) -> None:
    ...
    assert runtime.project_dir == project_root
    assert runtime.openfold_repo_dir == (project_root / "openfold-3").resolve()
    assert runtime.openfold_prefix == (project_root / ".venv").resolve()
    assert runtime.results_dir == (project_root / "results").resolve()
    assert runtime.msa_cache_dir == (
        project_root / "msa_cache" / "colabfold_msas"
    ).resolve()
    assert runtime.triton_cache_dir == (
        project_root / ".runtime" / "triton_cache"
    ).resolve()
    assert runtime.fixed_msa_tmp_dir == (
        project_root / ".runtime" / "of3_colabfold_msas"
    ).resolve()
...
def test_run_prediction_full_mocked_flow(monkeypatch, tmp_path: Path) -> None:
    ...
    class FakeMonitor:
        ...
        def stop(self):
            ...
            self.stage_marks_path.write_text(
